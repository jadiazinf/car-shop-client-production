...
function DatatableComponent(props: IDatatableComponentProps<any>) {
      ...
      case "actions":
        return (
          <div className="relative flex justify-center items-center gap-2">
            <Dropdown className="bg-background border-1 border-default-200" radius='sm'>
              <DropdownTrigger>
              ...
              <DropdownMenu
                ...
                onSelectionChange={(key) => {
                  const action = key as unknown as "view" | "update" | "delete";
                  props.setActionState ? props.setActionState({action: Array.from(action)[0] as "view" | "update" | "delete", id: user.id}) : null;
                }}
              >
                {
  ...
  return (
    <Table
      ...
      selectedKeys={Array.isArray(props.selectedData) ? props.selectedData.map( element => element.toString() ) : "all"}
      //@ts-ignore
      onSelectionChange={(keys) => keys === "all" ? props.setSelectedData("all") : props.setSelectedData(Array.from(keys).map(element => parseInt(element)))}
