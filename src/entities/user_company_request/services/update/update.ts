...
class UpdateUserCompanyRequestService {
  ...
  constructor(data: {request: any, user_company_request_id: number, company_id: number}) {
  ...
  public async perform() {
    try {
      const response = await axios.patch<UpdateRequestResponse | string[]>(`${EnvironmentVariables.API_BASE_ROUTE}/api/${EnvironmentVariables.API_VERSION}/users_companies_requests/${this._user_company_request_id}`,
        {
          user_company_request: {...this._request, company_id: this._company_id}
        },
        {
          headers: {
