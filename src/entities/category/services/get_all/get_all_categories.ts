...
class GetAllCategoryService {
  ...
  private _payload: PaginatedData<CategoryModel> | CategoryModel[] | null;
  ...
  public async perform(props?: {page: number}) {
    try {
      const response = await axios.get<PaginatedData<CategoryModel> | CategoryModel[]>(`${EnvironmentVariables.API_BASE_ROUTE}/api/${EnvironmentVariables.API_VERSION}/categories${props ? `?page=${props.page}` : ''}`,
      {
        headers: {
      ...
      this._errorMessage = response.status === StatusCodes.OK ? null : response.status === StatusCodes.NOT_FOUND ? "Categorias no encontrada" : "Error al buscar las categorias"
    } catch(error: any) {
