...
import { ENGINE_TYPE_ARR, VEHICLE_TRANSMISSION_ARR, VEHICLE_TYPES_ARR } from "../../../types";
...
function VehicleInfoForm(props: { initialValues: VehicleModel, onSubmit: (values: VehicleModel) => void, children?: ReactNode } ) {
  ...
  const formik = useFormik({initialValues: props.initialValues, onSubmit, validationSchema: VehicleInfoSchema});
  ...
  return (
    <form className='flex flex-col gap-5' onSubmit={formik.handleSubmit}>
      ...
      <Container>
        <SelectComponent
          label="Color"
          data={VEHICLE_COLORS.map(element => ({key: element, label: element}))}
          key="color"
          name="color"
        ...
        <TextComponent
          ...
          isError={formik.errors.license_plate && formik.touched.license_plate}
      ...
      <Container>
        <SelectComponent
          label="Año"
          data={DatesHelpers.getYearsDescending().map(year => ({key: year.toString(), label: year.toString()}))}
          key="year"
          name="year"
          onChange={formik.handleChange}
        ...
        <SelectComponent
          ...
          data={[1, 2, 3, 4, 5, 6].map( element => ({key: element.toString(), label: element.toString()}) )}
          key="axles"
          name="axles"
          onChange={formik.handleChange}
      ...
      <Container>
        ...
        <TextComponent
          ...
          isError={formik.errors.load_capacity && formik.touched.load_capacity}
      ...
      <Container>
        <TextComponent
          ...
          isError={formik.errors.engine_serial && formik.touched.engine_serial}
      ...
      <Container>
        <SelectComponent
          ...
          onBlur={formik.handleBlur}
          data={[2, 3, 4, 5, 6, 7, 8].map( element => ({key: element.toString(), label: element.toString()}) )}
        />
      </Container>
