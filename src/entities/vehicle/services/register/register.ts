...
class RegisterVehicleService {
  ...
  public async perform() {
    try {
      ...
      const response = await axios.post<ApplicationResponse<VehicleModel>>(
        `${EnvironmentVariables.API_BASE_ROUTE}/api/${EnvironmentVariables.API_VERSION}/vehicles`,
        {
          formData,
        },
        {
      ...
      this._errorMessage = Array.isArray(response.data.errors)
      ...
      this._errorMessage = Array.isArray(error.response.data.errors)
  ...
  private transformToFormData() {
    ...
    if (this._vehicle.color) formData.append("color", this._vehicle.color);
