...
} from "../../../entities/user/services/companies/use_get_companies";
...
import useGetUserCompanyRolesService, {
  GetUserCompanyRolesProps,
} from "../../../entities/users_companies/services/user_company_roles/get/use_service";
...
function ChooseUserCompanyPage() {
  ...
  const { authReducer } = useSelector(
    (state: RootState): PersistedStore => state.persistedReducer
  );
  ...
  const {
    ...
    payloadState: userCompanyRolesPayload,
    performGetRole,
  } = useGetUserCompanyRolesService();
  ...
  function setUserCompanyRoles() {
    const { payload, status } =
      userCompanyRolesPayload as unknown as GetUserCompanyRolesProps;

    if (status === StatusCodes.OK || status === StatusCodes.NOT_MODIFIED) {
      if (payload.length === 0) {
        toasterDispatch({
          payload:
            "No tiene rol definido para dicha compañía, por favor comuníquese con el administrador",
          type: "ERROR",
        });
    ...
    toasterDispatch({
      payload: "Error al obtener los roles para la companía seleccionada",
      type: "ERROR",
    });
    navigate("/auth/logout");
    return;
  ...
  useEffect(() => {
    if (
      !sessionType ||
      (sessionType.roles?.length === 1 &&
        sessionType.roles?.includes(UserCompanyRole.GENERAL))
    ) {
      navigate("/");
      return;
    }
    performGetUserCompanies(
      { user_id: sessionType!.user.id!, token: token! },
      handler
    );
  }, []);

  useEffect(() => {
    if (userCompanyRolesPayload !== "not loaded") setUserCompanyRoles();
  }, [userCompanyRolesPayload]);

  return (
    <div className="w-full h-full-w-footer flex flex-col gap-10">
      <p className="font-semibold font-inter text-2xl">
        Seleccione un taller para la sesión
      </p>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-5 gap-5">
        ...
        isGettingUserCompanyRolesLoading ? (
          <Spinner />
        ) : payloadState.status !== StatusCodes.OK ? (
      ...
      {isGettingUserCompaniesLoading ||
      payloadState === "not loaded" ||
      isGettingUserCompanyRolesLoading ? null : (
        <div className="w-full flex justify-center items-center">
          <div className="mt-5 justify-center items-center w-52">
            <ButtonComponent
              color="primary"
              text="Regresar"
              type="button"
