...
import useCreateCompany, { CreateCompanyProps } from "../../../../entities/company/services/create/use_create";
...
function Main() {
  ...
  const { isCreatingCompanyLoading, payloadState: companyPayload, performCreateCompany } = useCreateCompany();
  ...
  async function handleCompanyService() {
    if ((companyPayload as CreateCompanyProps).status !== StatusCodes.OK && (companyPayload as CreateCompanyProps).status !== StatusCodes.CREATED)
      toasterDispatch({payload: (companyPayload as CreateCompanyProps).errorMessage || 'Error al registrar compañía', type: 'ERROR'});
    else
      toasterDispatch({payload: "Compañía registrada con éxito", type: 'SUCCESS'});
  ...
  useEffect(() => {
    if (companyPayload !== 'not loaded')
      handleCompanyService();
  }, [companyPayload]);
    ...
    <>
      <div className='w-full h-full flex flex-col mt-10'>
        <Breadcrumbs underline="active" onAction={(key) => setRegisterStage(key as PageStage)}>
          ...
          <BreadcrumbItem key={PageStage.WORKSHOP} isCurrent={registerStage === PageStage.WORKSHOP} startContent={<RiToolsFill />}>
          ...
          <BreadcrumbItem key={PageStage.CONFIRM_WORKSHOP_INFO} isCurrent={registerStage === PageStage.CONFIRM_WORKSHOP_INFO} startContent={<BsPatchCheckFill />}>
              ...
              <div className='w-full h-full flex justify-center items-center flex-col gap-5'>
                {
                  ...
                  registerStage === PageStage.WORKSHOP ?
                    <div className='w-full flex flex-col gap-3'>
                      <div>
                        <span className='font-inter font-bold text-2xl'>Formulario de registro de taller</span>
                        <div className='hidden md:block md:w-1/6 bg-blue-600 h-1'></div>
                      </div>
                      <CompanyInfoForm
                        ...
                        onSubmit={(values) => {
                          setCompany(values);
                          setRegisterStage(status === AuthStatus.NOT_AUTHENTICATED ? PageStage.CONFIRM_USER_INFO : PageStage.CONFIRM_WORKSHOP_INFO);
                        }}
                      >
                        <div className='w-full flex justify-center items-center'>
                        ...
                        <div className='flex flex-col gap-5'>
                          ...
                          <div className='w-full flex flex-col gap-3'>
                            <ButtonComponent
                              ...
                              type="button"
                              variant="solid"
                              onClick={() => setRegisterStage(PageStage.CONFIRM_WORKSHOP_INFO)}
                            />
                            <ButtonComponent
                              color="primary"
                              text="Regresar"
                          ...
                          <div className='w-full flex flex-col gap-5'>
                            ...
                            <div className='w-full flex flex-col gap-3'>
                              <ButtonComponent
                                ...
                                type="button"
                                variant="solid"
                                onClick={status === AuthStatus.NOT_AUTHENTICATED ? handleRegisterUser : () => performCreateCompany({
                                  company: company!,
                                  token: token!,
                                  user_id: sessionType!.user.id!
                                })}
                                isLoading={isRegisteringUserLoading || isCreatingCompanyLoading}
                              />
                              <ButtonComponent
                                color="primary"
                                text="Regresar"
                                type="button"
                                variant="light"
                                onClick={() => setRegisterStage(status === AuthStatus.NOT_AUTHENTICATED ? PageStage.CONFIRM_USER_INFO : PageStage.WORKSHOP)}
                                isDisabled={isRegisteringUserLoading || isCreatingCompanyLoading}
                              />
                            </div>
                          </div>
                        :
