...
import useLogoutService, {
  LogoutServiceData,
} from "../../../auth/services/logout/use_logout";
...
function LogoutPage() {
  ...
  function handler(data: LogoutServiceData) {
    if (data.status !== StatusCodes.OK) {
      toasterDispatch({
        payload: data.errorMessage || "Error al cerrar sesión",
        type: "ERROR",
      });
      return;
    }
    ...
    toasterDispatch({
      payload: "Sesión cerrada exitosamente",
      type: "SUCCESS",
    });
  }
  ...
  return <WindowLoader message="Cerrando sesión" />;
}
