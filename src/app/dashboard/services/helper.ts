...
class DashboardServicesAdminPageHelpers {
  static addOrRemoveOrSetElements(props: {
    selectedValues: number[] | "all";
    newServices: NewServiceInfo;
  }): "all" | "remove" | number {
    const { selectedValues: sv, newServices: ns } = props;

    if (sv === "all") return "all";
  ...
  static arrWithRemovedService(props: {
    services: number[];
    newServices: NewServiceInfo;
  }) {
    let arr: { id: number; price: number }[] = [];
    props.services.forEach(
      (service) =>
        props.newServices.services.some((element) => element.id === service) &&
        arr.push(
          props.newServices.services.find((element) => element.id === service)!
        )
    );
    return arr;
  }

  static arrWithNewService(props: {
    services: NewServiceInfo;
    newService: { id: number; price: number };
  }) {
