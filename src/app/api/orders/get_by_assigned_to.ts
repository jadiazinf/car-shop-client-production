...
export function getOrdersByAssignedTo() {
  ...
  async function perform(props: RequestProps, token: string) {
    try {
      ...
      const response = await api.get<PaginatedData<OrderModel> | ErrorResponse>(
        `${API_BASE_ROUTE_EXTENSION}/orders/by_assigned_to?page=${props.page}${
        ...
        }${props.assigned_to ? `&assigned_to=${props.assigned_to}` : ""}`,
        {
          headers: {
  ...
  return {
    perform,
    isGettingOrders,
    getOrdersByAssignedToResponse: response,
  };
}
