...
export function getCompanyRequests() {
  ...
  async function perform(company_id: number, token: string, filters: {status?: string, page?: number}) {
    try {
      setIsGettingRequests(true);

      const response = await api.get<PaginatedData<UserCompanyRequestModel> | ErrorResponse>(
        `${API_BASE_ROUTE_EXTENSION}/users_companies_requests/show_by_company_id?company_id=${company_id}${filters ? `&${new URLSearchParams(Object.entries(filters).reduce((acc, [key, value]) => {
  ...
  return {
    perform,
    isGettingRequests,
    getCompanyRequestsResponse: response,
  };
}
