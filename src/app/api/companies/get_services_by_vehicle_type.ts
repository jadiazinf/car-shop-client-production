...
export function getCompanyServicesByVehicleType() {
  ...
  async function perform(
    ...
    try {
      ...
      const response = await api.get<PaginatedData<ServiceModel>>(
        `${API_BASE_ROUTE_EXTENSION}/companies/${company_id}/services_by_vehicle_type?page=${page}&vehicle_type=${vehicle_type}`
      );
      setResponse({
        data: {
          ...response.data,
  ...
  return {
    ...
    getCompanyServicesByVehicleTypeResponse: response,
  };
}
